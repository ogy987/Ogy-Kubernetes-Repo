# How to deploy Jenkins to Kubernetes cluster on AWS using Helm charts
...
2) At least **8GB of free memory** and **2 vCPU** in the Kubernetes cluster for Jenkins microservices. An **`m5.large`** instance should do the job.
...
## STEP 2 – Create a Route53 domain for Jenkins
...
## STEP 4 –  Setup Ingress for Jenkins

- After we install NGINX Ingress controller, we need to create **Record Set** inside Route53 `getswift-testing.co` hosted zone for our previously created Jenkins DNS domain and configure the ingress routing.

- In Route53, choose `getswift-testing.co`, find Jenkins record, select it and then goes to **Create Record → Routing policy: Simple routing → Define simple record → Record name: → Value/Route traffic to: Alias to Application and Classic Load Balancer → Choose region: → Choose load balancer: →  Record type: A → Define simple record**.
...
## STEP 7 – Deploy Jenkins using Helm charts

- We will now install Jenkins using the bitnami's Helm chart and customized `values.yaml` file:
...
```bash
	...
	$ helm install jenkins bitnami/jenkins -f jenkins-values-SSL.yaml
```
- This outputs something similar to the following:

```bash
NAME: jenkins
LAST DEPLOYED: Mon Oct 26 17:26:08 2020
NAMESPACE: jenkins
STATUS: deployed
REVISION: 1
NOTES:
...
- In order to verify that your Jenkins installation was successfully deployed, run the command:

```bash
	$ kubectl get pods
```

- Output like this confirms the successful installation of Jenkins:

```bash
NAME                                READY   STATUS      RESTARTS   AGE
jenkins-575ccf4869-jhsz2            1/1     Running     0          11m
```

- You can check it by accessing your Jenkins URL inside the web browser. Also, you may notice that Jenkins require username and password for Sign In.

- You can retrieve username and password, requred for login, with the following commands:
...
- After you got the required username and password credential from the previous commands, you can successfully Sign-In into Jenkins.
